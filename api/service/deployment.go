...
// ScaleDeployment scales an application by creating a new deployment with updated resources
func (s *DeploymentServer) ScaleDeployment(
	ctx context.Context,
	req *connect.Request[deploymentv1.ScaleDeploymentRequest],
) (*connect.Response[deploymentv1.ScaleDeploymentResponse], error) {
	r := req.Msg
	...
	resources := &deploymentv1.ResourceSpec{}
	if resourceData, ok := config["resources"].(map[string]any); ok {
		if cpu, ok := resourceData["cpu"].(string); ok && cpu != "" {
			resources.Cpu = &cpu
		}
		if memory, ok := resourceData["memory"].(string); ok && memory != "" {
	...
	deployment, err := s.queries.CreateDeployment(ctx, genDb.CreateDeploymentParams{
		AppID:         r.AppId,
		ClusterID:     1, // todo: hardcoded currently.
	...
	return connect.NewResponse(&deploymentv1.ScaleDeploymentResponse{
		Deployment: deploymentResp,
	}), nil
}

// UpdateDeploymentEnv updates environment variables for an application
func (s *DeploymentServer) UpdateDeploymentEnv(
	ctx context.Context,
	req *connect.Request[deploymentv1.UpdateDeploymentEnvRequest],
) (*connect.Response[deploymentv1.UpdateDeploymentEnvResponse], error) {
	r := req.Msg
	...
	return connect.NewResponse(&deploymentv1.UpdateDeploymentEnvResponse{
		Deployment: deploymentResp,
	}), nil
}
